...
export default function AboutPage() {
  ...
  return (
    <main className="min-h-screen bg-[var(--color-astrawhite)]">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-8 md:gap-12 items-center">
            <div className="order-2 md:order-1">
              ...
              <div className="space-y-4 md:space-y-8 text-justify">
                <p className="text-[var(--color-astrablack)] text-base md:text-lg max-w-3xl mx-auto leading-relaxed">
      ...
      <section className="w-full py-8 md:py-12 bg-[#EFF2FA]">
      <div className="px-4 sm:px-6 md:px-8 max-w-7xl mx-auto py-8 md:py-12 text-[#061A37]">
        <div className="container px-4 md:px-6 mx-auto">
          <div className="flex flex-col items-center text-center mb-8 md:mb-10 space-y-3 md:space-y-4">
            <div className="flex items-center space-x-2">
              <Rocket className="h-6 w-6 md:h-8 md:w-8 text-[#0E6CF3]" />
              <h2 className="text-2xl md:text-3xl lg:text-4xl xl:text-5xl font-bold text-[#061A37]">
                ICS-ASTRA
              </h2>
            </div>
            <p className="max-w-[700px] text-[#526683] text-sm md:text-base lg:text-xl px-2">
              Meet the brilliant minds behind our mission to connect with the alumni of the Institute of Computer Science at UPLB.
            </p>
            <div className="flex gap-1 md:gap-2">
              {Array(5)
                .fill(0)
                .map((_, i) => (
                  <Star key={i} className="h-4 w-4 md:h-5 md:w-5 text-[#0E6CF3]" fill="currentColor" />
                ))}
            </div>
          </div>
            ...
            {Object.keys(contributors).map((category) => (
              <button
                ...
                className={`
                  ...
                  border border-[#99AFCE]
                  ${activeTab === category ? "bg-[#0E6CF3] text-white" : "bg-[#F3F5F9] text-[#526683]"}
          ...
          <div className="grid grid-cols-2 md:grid-cols-4 gap-3 md:gap-4">
            {contributors[activeTab].map((contributor, index) => (
              <Card
                key={index}
                className="bg-[#FFFFFF] border-[#E4E5E5] overflow-hidden transform transition-all duration-200 hover:scale-105 hover:shadow-lg hover:shadow-[#0E6CF3]/20"
              >
                <CardContent className="p-4 md:p-6">
                  <div className="flex flex-col items-center text-center space-y-3 md:space-y-4">
                    <div className="relative">
                      <div className="absolute inset-0 bg-[var(--color-astrawhite)] rounded-full blur-sm opacity-70"></div>
                      <div className="relative w-16 h-16 md:w-24 md:h-24 rounded-full overflow-hidden border-2 border-[#0E6CF3]">
                        <img
                    ...
                    <div className="space-y-1">
                      <h3 className="text-base sm:text-lg md:text-xl font-bold text-[#061A37]">{contributor.name}</h3>
                      <p className="text-xs sm:text-sm md:text-base text-[#526683]">{contributor.role}</p>
                    </div>
                  </div>
                </CardContent>
              </Card>
