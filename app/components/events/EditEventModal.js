...
export default function EditEventModal({ event, onClose, onSave }) {
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center">
      ...
      {/* Modal Box */}
      <div className="relative z-50 bg-astrawhite p-6 rounded-lg shadow-xl w-[90%] max-w-md">
        ...
        <form onSubmit={handleSubmit} className="space-y-4">
          ...
          <div className="flex justify-end gap-2 pt-2">
            <button
              type="button"
              onClick={onClose}
              className="px-4 py-2 border border-astradarkgray rounded-md"
            >
              Cancel
            </button>
            <button
              type="submit"
              className="px-4 py-2 bg-astraprimary text-astrawhite rounded-md"
            >
              Save
            </button>
          </div>
